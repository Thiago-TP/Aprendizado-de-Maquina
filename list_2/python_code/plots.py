...
def logistic_regression_postprocessing(
    ...
    _plot_roc_f1_cm_logistic_regression(
        output_folder=output_folder, thresholds=thresholds, th_optimal=0.35
    )
...
def _plot_roc_f1_cm_logistic_regression(
    output_folder: str = "./plots/logistic_regression/",
    thresholds: np.ndarray = np.arange(0.1, 0.91, 0.01),
    th_optimal: float = 0.35,
) -> None:
    """
    Plots logistic regression ROC curve, F1-score for both training and testing.
    Also plots the confusion matrix for the optimal threshold on the test set.
    """
    for mode, color in zip(["train", "test"], ["tab:blue", "tab:orange"]):
        ...
        for th in thresholds:
            ...
            tp = np.sum(  # true positives
                (predictions["probability_male"] >= th) & (truth["male_label"] == 1)
            )
            fn = np.sum(  # false negatives
                (predictions["probability_male"] < th) & (truth["male_label"] == 1)
            )
            fp = np.sum(  # false positives
                (predictions["probability_male"] >= th) & (truth["male_label"] == 0)
            )
            tn = np.sum(  # true negatives
                (predictions["probability_male"] < th) & (truth["male_label"] == 0)
            )
        ...
        # F1-Score
        f1_ax = f1_df.plot("threshold", "score", ax=axs[0], **style)
        f1_ax.set_ylabel("Value (%)", fontsize=18)
        f1_ax.set_xlabel("Threshold", fontsize=18)
        f1_ax.set_title("F1-Score", fontsize=18)

        # ROC
        roc_ax = roc_df.plot("fpr", "tpr", ax=axs[1], **style)
        roc_ax.set_ylabel("True Positive Rate (%)", fontsize=18)
        roc_ax.set_xlabel("False Positive Rate (%)", fontsize=18)
        roc_ax.set_title("ROC", fontsize=18)
